...
export default {
    ...
    fields: [
        ...
        },
        {
            ...
            title: '时间',
            type: 'string',
            description: "事件时间。请用24小时制HH:MM表示，例如14：02。如果未知，则无需填写。若不清楚分钟，请用XX代替，例如14:XX。",
            validation: (Rule: Rule) => Rule.custom((time: Nullable) => {
                ...
                if (isNaN(Number(minute))) {
                    if (minute != "XX") {
                        return "如果不清楚分钟，请用'XX'代替，你输入了" + minute
                    }
                } else {
        ...
        },
        {
            name: 'tags',
            title: 'Tags',
            description: "事件tag，例如'当事人声明'，可随意填写1-2个tags，填写完按回车键确定",
            type: 'array',
            of: [{ type: 'string' }],
            options: {
                layout: "tags"
